function [A_hat, U_hat, V_hat] = inexact_alm_rpca(D, lambda, tol, maxIter)
...
% initialize
Y = D;
norm_two = norm(Y,2);%lansvd(Y, 1, 'L');
norm_inf = norm( Y(:), inf) / lambda;
dual_norm = max(norm_two, norm_inf);
Y = Y / dual_norm;

A_hat = zeros( m, n, 'gpuArray');
E_hat = zeros( m, n, 'gpuArray');
...
%     end
    ...
    U_hat = U(:, 1:svp) * diag(diagS(1:svp) - 1/mu).^.5;    
    V_hat = V(:, 1:svp) * diag(diagS(1:svp) - 1/mu).^.5;    
